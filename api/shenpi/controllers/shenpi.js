...
module.exports = (config) => {
  ...
  const shenpiLiuchengSave = async (req, res, next) => {
    if (req.hooks.mShenpi && req.hooks.neirong) {
      ...
      for (const one of req.toUserNames) {
        const tmingxi = await mShenpiMingxi.findOne({ where: { shenpiId: req.params.shenpiId, userId: one.userId, index: one.index, roleId: one.roleId } });
        if (tmingxi) {
  ...
  const list = [
    ...
    async (req, res, next) => {
      ...
      const neirongs = req.params.haveNeirong ? (await model(`${name}Neirong_${(req.params.shenpiType || '').toLowerCase()}`).findAll({ where: { shenpiId: { $in: workIds } } })).map(one => (one.get())) : [];
  ...
  const detail = [
    ...
    async (req, res, next) => {
      ...
      const neirongList = item.neirongType === '多条内容' ? await req.mShenpiNeirong.findAll({ where: { shenpiId: item.id } }) : await req.mShenpiNeirong.findOne({ where: { shenpiId: item.id } });
