...
export default ({ U: { models }, config: { service, upload } }) => ({
  ...
  formatDbField: (model, fieldName, type, defaultVal = []) => {
  ...
  baseField: {
    ...
    createdAt: {
      ...
      get() {
        return this.formatDbField(this, 'createdAt', 'datetime');
      },
    },
    updatedAt: {
      type: Sequelize.DATE,
      ...
      defaultValue: Sequelize.NOW,
      get() {
        return this.formatDbField(this, 'updatedAt', 'datetime');
      },
    },
    deletedAt: {
      type: Sequelize.DATE,
      ...
      defaultValue: null,
      get() {
        return this.formatDbField(this, 'deletedAt', 'datetime');
      },
    },
  },
    ...
    for (let i = 0; i < checkUnique.length; ++i) {
      ...
      if (haveChange) {
        if (!(Array.isArray(one.noCheck) && one.noCheck.indexOf(tempCheckValue.join(',-,')) >= 0)) {
          ...
          const results = await models(model._modelOptions.name.singular).find({ where });
          if (results) {
